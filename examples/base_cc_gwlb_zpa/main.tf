...
locals {
  ...
  # Read SSH public key from file if provided, otherwise use auto-generated key
  public_key = var.ssh_public_key_file != null ? file(var.ssh_public_key_file) : try(tls_private_key.key[0].public_key_openssh, "")
}
...
# For production deployments, set ssh_public_key_file variable to pass your
# own custom public key file located in a secure location.
################################################################################
